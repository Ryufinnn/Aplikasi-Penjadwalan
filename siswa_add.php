...
if(isset($_POST['btnSimpan'])){
	...
	if (count($pesanError)>=1 ){
		...
		$mySql	= "INSERT INTO siswa ( 
								...
								kelamin,kelas,wali_kelas, agama, tempat_lahir, 
								tanggal_lahir, alamat, no_telepon, 
								foto, tahun_angkatan, status)
							VALUES ( 
									'$kodeBaru', '$txtNis', '$txtNama', 
									'$cmbKelamin','$_POST[nim]','$_POST[nm]', '$cmbAgama', '$txtTempatLahir', 
...
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="post" enctype="multipart/form-data" target="_self">
  <table class="table-list" width="100%" border="0" cellpadding="3" cellspacing="1">
    ...
    while ($row = mysql_fetch_array($result)) {   
        ...
        $jsArray .= "dtMhs['" . $row['nama_kelas'] . "'] = {nama:'" . addslashes($row['wali_kelas']) . "',jrsn:'".addslashes($row['rujukan'])."'};\n";   
    }     
    ?>   
        </select>        </td>
    </tr>
	<tr>
      ...
      <td><b>:</b></td>
      <td> <input type='text' data-field="x_username" id="nm" name='nm' class='required' placeholder="Wali Kelas">      </td>
    </tr>
    <tr>
      <td><b>Agama</b></td>
      <td><b>:</b></td>
...
<script type="text/javascript">   
    <?php echo $jsArray; ?> 
    function changeValue(kode_kelas){ 
    document.getElementById('nm').value = dtMhs[kode_kelas].nama; 
    document.getElementById('jrsn').value = dtMhs[kode_kelas].jrsn; 
    }; 
    </script> 
